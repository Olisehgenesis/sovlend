  ...
  return (
    <div className="app-shell">
      <aside className="sidebar">
        <div className="brand"><SovLendMark /><span>SovLend</span></div>
        <div className="workspace"><span>{dashboard.organizationName}</span><small>{dashboard.officeName}</small></div>
        <nav aria-label="Primary navigation">
          {nav.map(([Icon, label, href]) => <a className={href === "/" ? "active" : ""} href={href} key={label}><Icon size={18} /><span>{label}</span></a>)}
        </nav>
        <div className="sidebar-bottom">
          {session.user.role === "admin" ? <><a href="/backoffice"><ShieldCheck size={18} /><span>Backoffice</span></a><a href="/admin/users"><ShieldCheck size={18} /><span>Users &amp; access</span></a></> : null}
          <a href="/settings/security"><Settings size={18} /><span>Security</span></a>
          <AccountMenu />
        </div>
      </aside>
      ...
      <main>
        <header className="topbar">
          <div className="topbar-context"><span>Workspace</span><strong>{dashboard.organizationName}</strong><small>{dashboard.officeName}</small></div>
          <nav className="topbar-actions" aria-label="Quick actions"><Link href="/clients">Find a client</Link><Link href="/loans">Open lending</Link></nav>
        </header>
