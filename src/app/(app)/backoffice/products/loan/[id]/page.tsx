...
export default async function EditLoanProductPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <main className="directory-page">
    ...
    <section className="panel form-panel">
      ...
      <EditLoanProductForm product={{ id: product.id, name: product.name, shortName: product.shortName, denominationCurrency: product.denominationCurrency, principalMin: formatMinorInput(product.principalMinMinor), principalMax: formatMinorInput(product.principalMaxMinor), annualRate: formatMonthlyPercent(product.annualRateBps), monitoringFeeAnnualRate: formatMonthlyPercent(product.monitoringFeeAnnualRateBps), repaymentCount: product.repaymentCount, repaymentFrequency: product.repaymentFrequency, amortizationMethod: product.amortizationMethod, interestMethod: product.interestMethod === "Declining Balance" || product.interestMethod === "DECLINING_BALANCE" ? "Declining Balance" : "Flat", collectCrb: product.collectCrb }} />
    </section>
  </main>;
}
