...
export default async function EditSavingsProductPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <main className="directory-page">
    <Breadcrumbs items={[{ label: "Backoffice", href: "/backoffice" }, { label: "Products", href: "/backoffice/products" }, { label: displaySavingsProductName(product.name) }, { label: "Edit" }]} />
    ...
    <section className="panel form-panel">
      <div className="panel-heading"><div><h2>{displaySavingsProductName(product.name)}</h2><p>{product.shortName} · {product.active ? "Active" : "Inactive"}</p></div></div>
