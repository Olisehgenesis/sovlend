...
import { CreateChargeDefinitionForm, CreateLoanProductForm, CreateSavingsProductForm } from "@/components/product-forms";
...
export default async function ProductsPage() {
  ...
  return <main className="directory-page">
    ...
    <section className="panel">
      <div className="panel-heading"><div><h2>Loan products</h2><p>Credit templates used when pricing, approving and disbursing loans.</p></div></div>
      {loanProducts.length === 0 ? <div className="empty-state compact-empty"><strong>No loan products yet</strong></div> : <div className="table-scroll"><table className="clickable-rows"><thead><tr><th>Name</th><th>Principal range</th><th>Nominal rate</th><th>Monitoring fee</th><th>Repayments</th><th>Status</th><th></th></tr></thead><tbody>{loanProducts.map((product) => <tr key={product.id}><td><strong>{product.name}</strong><small>{product.shortName}</small><Link aria-label={`Open loan product ${product.name}`} className="row-link" href={`/backoffice/products/loan/${product.id}`} /></td><td>{formatMinor(product.principalMinMinor, product.denominationCurrency)}{"–"}{formatMinor(product.principalMaxMinor, product.denominationCurrency)}</td><td>{(product.annualRateBps / 100).toFixed(2)}%</td><td>{(product.monitoringFeeAnnualRateBps / 100).toFixed(2)}%</td><td>{product.repaymentCount}{"×"}{product.repaymentFrequency.toLowerCase()}</td><td><span className={`status ${product.active ? "up-to-date" : "review"}`}>{product.active ? "Active" : "Inactive"}</span></td><td><ArchiveToggleButton active={product.active} url={`/api/backoffice/loan-products/${product.id}`} /></td></tr>)}</tbody></table></div>}
