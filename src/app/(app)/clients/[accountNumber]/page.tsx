...
export default async function ClientDetailPage({ params, searchParams }: { params: Promise<{ accountNumber: string }>; searchParams: Promise<{ tab?: string }> }) {
  ...
  const savingsProductLabel = (account: { product: { name: string } | null; accountType: string }) =>
    displaySavingsProductName(account.product?.name ?? account.accountType.replaceAll("_", " "));
