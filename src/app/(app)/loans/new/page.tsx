...
export default async function NewLoanApplicationPage({ searchParams }: { searchParams: Promise<{ query?: string; clientId?: string }> }) {
  ...
  const [clients, selectedClient, products] = await Promise.all([
  ...
  return <main className="directory-page"><Breadcrumbs items={[{ label: "Loans", href: "/loans" }, { label: "New application" }]} /><header className="directory-header"><div><p className="eyebrow">Loan origination</p><h1>New loan application</h1><p>Choose an active borrower and product. A different authorized user must approve it.</p></div><Link className="secondary-action" href="/loans">Cancel</Link></header><form className="directory-search"><Search size={17} /><input name="query" defaultValue={query} placeholder="Find active client by name, account or mobile" /><button type="submit">Find client</button></form>{availableClients.length === 0 ? <section className="panel"><div className="empty-state"><Search size={28} /><strong>No active clients found</strong><p>Try another name, account number, or mobile number.</p><Link className="invest-button empty-action" href="/clients/new">Create client</Link></div></section> : <section className="panel form-panel"><CreateLoanApplicationForm clients={availableClients.map((client) => ({ id: client.id, name: [client.firstName, client.middleName, client.lastName].filter(Boolean).join(" "), accountNumber: client.accountNumber }))} products={products.map((product) => ({ id: product.id, name: product.name, currency: product.denominationCurrency, minimum: formatMinor(product.principalMinMinor, product.denominationCurrency).replace(`${product.denominationCurrency} `, ""), maximum: formatMinor(product.principalMaxMinor, product.denominationCurrency).replace(`${product.denominationCurrency} `, "") }))} selectedClientId={selectedClient?.id} /></section>}</main>;
}