...
export async function GET(request: Request) {
  ...
  if (!allowed) return NextResponse.json({ error: "You cannot view the Balance Sheet report" }, { status: 403 });
  ...
  const report = await getBalanceSheetReport(prisma, scope, {
