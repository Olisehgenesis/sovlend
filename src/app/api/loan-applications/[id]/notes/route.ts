...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (error instanceof PermissionDeniedError) {
      return NextResponse.json({ error: "You cannot add notes for this application" }, { status: 403 });
    }
    throw error;
  }

  const note = await prisma.loanApplicationNote.create({
    data: { applicationId: application.id, authorId: session.user.id, body: parsed.data.body },
  });
