...
export async function POST(request: Request) {
  ...
  const [client, group, product, loanOfficer, fund] = await Promise.all([
    prisma.client.findFirst({ where: { id: parsed.data.clientId, organizationId: scope.organizationId } }),
    parsed.data.groupId
      ? prisma.group.findFirst({
          where: { id: parsed.data.groupId, organizationId: scope.organizationId },
          include: { members: { where: { clientId: parsed.data.clientId }, select: { id: true } } },
        })
      : null,
  ...
  if (parsed.data.groupId) {
    ...
    if (group.status !== "ACTIVE") return NextResponse.json({ error: "Only active groups can be selected" }, { status: 400 });
    if (group.members.length === 0) return NextResponse.json({ error: "Selected client is not a member of that group" }, { status: 400 });
  }
