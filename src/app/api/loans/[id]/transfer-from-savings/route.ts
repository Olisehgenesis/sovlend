...
// Internal transfer: pays down a loan directly from the client's own savings balance, with no
// settlement/cash account involved. See transferSavingsToLoan() for the accounting treatment.
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const transaction = await transferSavingsToLoan(prisma, {
    ...
    if (error instanceof PermissionDeniedError) return NextResponse.json({ error: "You do not have permission to transfer savings to a loan" }, { status: 403 });
