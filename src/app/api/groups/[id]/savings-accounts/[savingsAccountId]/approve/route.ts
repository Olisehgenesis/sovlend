...
export async function POST(_request: Request, { params }: { params: Promise<{ id: string; savingsAccountId: string }> }) {
  ...
  try {
    await new AuthorizationService(prisma).assertAllowed({ actorUserId: session.user.id, permission: permissions.savingsApprove, organizationId: scope.organizationId, officeId: group.officeId });
  } catch (error) {
    if (error instanceof PermissionDeniedError) return NextResponse.json({ error: "You cannot approve savings accounts for this group" }, { status: 403 });
    throw error;
  }

  const savingsAccount = await prisma.savingsAccount.findFirst({ where: { id: savingsAccountId, groupId: group.id } });
