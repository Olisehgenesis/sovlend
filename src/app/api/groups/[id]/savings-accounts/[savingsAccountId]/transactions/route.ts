...
export async function POST(request: Request, { params }: { params: Promise<{ id: string; savingsAccountId: string }> }) {
  ...
  try {
    await new AuthorizationService(prisma).assertAllowed({ actorUserId: session.user.id, permission: permissions.savingsTransact, organizationId: scope.organizationId, officeId: group.officeId });
  } catch (error) {
  ...
  const savingsAccount = await prisma.savingsAccount.findFirst({ where: { id: savingsAccountId, groupId: group.id }, select: { id: true } });
