...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string; chargeId: string }> }) {
  ...
  try {
    ...
    if (error instanceof PermissionDeniedError) return NextResponse.json({ error: "You cannot edit charges for this client" }, { status: 403 });
    throw error;
  }

  const charge = await prisma.charge.findFirst({ where: { id: chargeId, clientId: client.id } });
