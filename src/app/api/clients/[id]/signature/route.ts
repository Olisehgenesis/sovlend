...
export async function POST(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!client.signatureDocumentId) return NextResponse.json({ error: "No signature is on file" }, { status: 400 });

  await prisma.client.update({ where: { id: client.id }, data: { signatureDocumentId: null } });
