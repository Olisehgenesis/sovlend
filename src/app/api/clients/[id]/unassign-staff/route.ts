...
export async function POST(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!client.assignedOfficerId) return NextResponse.json({ error: "This client has no assigned staff" }, { status: 400 });

  await prisma.client.update({ where: { id: client.id }, data: { assignedOfficerId: null } });
