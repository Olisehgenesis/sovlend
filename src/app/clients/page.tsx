...
export default async function ClientsPage({ searchParams }: { searchParams: Promise<{ query?: string; page?: string }> }) {
  ...
  return <main className="directory-page"><header className="directory-header"><div><p className="eyebrow">Customer directory</p><h1>Clients</h1><p>{total.toLocaleString()} records in your office scope</p></div><div className="header-actions"><Link className="secondary-action" href="/"><span>Overview</span></Link><a className="secondary-action" href="/api/clients/export"><Download size={16} /> Export CSV</a><Link className="invest-button" href="/clients/new"><UserPlus size={16} /> Create client</Link></div></header><form className="directory-search"><Search size={17} /><input name="query" defaultValue={query} placeholder="Search name, account or mobile" /><button type="submit">Search</button></form><section className="panel"><div className="table-scroll"><table><thead><tr><th>Client</th><th>Account</th><th>Mobile</th><th>Office</th><th>Status</th><th>Action</th></tr></thead><tbody>{clients.map((client) => <tr key={client.id}><td><strong>{[client.firstName, client.middleName, client.lastName].filter(Boolean).join(" ")}</strong><small>{client.externalId ?? "No external ID"}</small></td><td className="mono">{client.accountNumber}</td><td>{client.mobileNumber ?? "Not provided"}</td><td>{client.office.name}</td><td><span className={`status ${client.status === "ACTIVE" ? "up-to-date" : "review"}`}>{client.status}</span></td><td>{client.status === "ACTIVE" ? <Link className="green-link" href={`/loans/new?clientId=${client.id}`}>New loan</Link> : <span className="muted-text">Activate first</span>}</td></tr>)}</tbody></table></div>{clients.length === 0 ? <div className="empty-state"><Users size={28} /><strong>No matching clients</strong><p>Change the search and try again.</p></div> : null}<nav className="pagination" aria-label="Client pages"><Link aria-disabled={page <= 1} href={`/clients?query=${encodeURIComponent(query)}&page=${Math.max(1, page - 1)}`}>Previous</Link><span>Page {page} of {pages}</span><Link aria-disabled={page >= pages} href={`/clients?query=${encodeURIComponent(query)}&page=${Math.min(pages, page + 1)}`}>Next</Link></nav></section></main>;
}