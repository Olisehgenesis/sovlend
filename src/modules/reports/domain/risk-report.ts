...
function agingLoanRow(loan: AgingReport["loans"][number]) {
  return {
    "Loan Account No.": loan.accountNumber,
    Client: loan.borrowerName,
    ...
    "Days Overdue": String(loan.daysOverdue),
    "Aging Bucket": loan.agingBucket,
  };
}
...
export function arrearsReportCsv(report: ArrearsReport) {
  return rowsToCsv(
    report.loans.map((loan) => ({
      ...
      "Outstanding Principal": loan.outstandingPrincipalMinor.toString(),
      "Total Outstanding": loan.outstandingTotalMinor.toString(),
      "Days Overdue": String(loan.daysOverdue),
      "Overdue Since": loan.overdueSince ? isoDate(loan.overdueSince) : "",
      "Aging Bucket": loan.agingBucket,
    })),
    [...arrearsLoanColumns],
  );
...
export function provisioningReportCsv(report: ProvisioningReport) {
  return rowsToCsv(
    report.loans.map((loan) => ({
      ...
      Status: loan.status,
      Currency: loan.currencyCode,
      "Total Outstanding": loan.outstandingMinor.toString(),
      "Days Overdue": String(loan.daysOverdue),
      "Provisioning Bucket": loan.provisioningBucket,
      "Provision Rate %": String(loan.provisionRatePercent),
      "Provision Amount": loan.provisionMinor.toString(),
    })),
    [
      "Loan Account No.",
      "Client",
...
export function recoveriesReportCsv(report: RecoveriesReport) {
  return rowsToCsv(
    report.loans.map((loan) => ({
      ...
      Product: loan.productName,
      Currency: loan.currencyCode,
      "Original Principal": loan.originalPrincipalMinor.toString(),
      "Write-Off Date": isoDate(loan.writeOffDate),
      "Written-Off Amount": loan.writeOffAmountMinor.toString(),
      "Recovered Amount": loan.recoveredAmountMinor.toString(),
      "Recovery Count": String(loan.recoveryCount),
      "Latest Recovery Date": loan.latestRecoveryOn ? isoDate(loan.latestRecoveryOn) : "",
    })),
    [
      "Loan Account No.",
      "Client",
...
export function parRollRateReportCsv(report: ParRollRateReport) {
  return rowsToCsv(
    report.rows.map((row) => ({
      ...
      Loans: String(row.loanCount),
      "Original Principal": row.originalPrincipalMinor.toString(),
      Current: row.distribution.CURRENT.principalMinor.toString(),
      "1-30 Days": row.distribution["1_30"].principalMinor.toString(),
      "31-60 Days": row.distribution["31_60"].principalMinor.toString(),
      "61-90 Days": row.distribution["61_90"].principalMinor.toString(),
      "90+ Days": row.distribution["90_PLUS"].principalMinor.toString(),
      "Written Off": row.distribution.WRITTEN_OFF.principalMinor.toString(),
    })),
    [
      "Cohort",
      "Loans",
