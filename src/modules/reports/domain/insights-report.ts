...
export function groupPortfolioReportCsv(report: GroupPortfolioReport) {
  return rowsToCsv(
    report.rows.map((row) => ({
      ...
      "Loan Officer": row.officerName ?? "Unassigned",
      Members: String(row.memberCount),
      "Savings Balance": row.savingsBalanceMinor.toString(),
      "Savings Accounts": String(row.savingsAccountCount),
      "Loan Portfolio": row.loanPrincipalMinor.toString(),
      "Total Loans": String(row.loanCount),
      "Active Loans": String(row.activeLoanCount),
      "Loans in Arrears": String(row.arrearsLoanCount),
      "Closed Loans": String(row.closedLoanCount),
      "Other Loans": String(row.otherLoanCount),
      "Outstanding Principal": row.outstandingPrincipalMinor.toString(),
...
export function guarantorExposureReportCsv(report: GuarantorExposureReport) {
  return rowsToCsv(
    report.rows.map((row) => ({
      Guarantor: row.displayName,
      Phone: row.phone,
      "Loans Backed": String(row.loanCount),
      "Loans in Arrears": String(row.arrearsLoanCount),
      "Arrears Concentration": String(row.concentrationRisk),
      "Total Exposure": row.totalOutstandingMinor.toString(),
      Currency: row.currencyCode,
      "Backed Loans": row.loans
        .map(
          (loan) =>
...
export function feeRevenueReportCsv(report: FeeRevenueReport) {
  return rowsToCsv(
    report.rows.map((row) => ({
      "Charge Type": row.name,
      Currency: row.currencyCode,
      Collected: row.amounts.PAID.toString(),
      "Collected Charges": String(row.counts.PAID),
      Outstanding: row.amounts.PENDING.toString(),
      "Outstanding Charges": String(row.counts.PENDING),
      Waived: row.amounts.WAIVED.toString(),
      "Waived Charges": String(row.counts.WAIVED),
      "Total Value": row.totalMinor.toString(),
    })),
    [
      "Charge Type",
      "Currency",
...
export function documentCompletenessReportCsv(report: DocumentCompletenessReport) {
  return rowsToCsv(
    report.rows.map((row) => ({
      ...
      "Client Account No.": row.borrowerAccountNumber ?? "",
      Status: row.status,
      "Document Count": String(row.documentCount),
      Documents: row.documentNames.join(" | "),
      "Present Core Docs": row.presentCategories.join(", "),
      "Missing Core Docs": row.missingCategories.join(", "),
    })),
    [
      "Loan Account No.",
      "Client",
...
export function auditTrailReportCsv(report: AuditTrailReport) {
  return rowsToCsv(
    report.rows.map((row) => ({
      ...
      "Audit Event ID": row.id,
      Actor: row.actorName,
      "Actor ID": row.actorId ?? "SYSTEM",
      Action: row.action,
