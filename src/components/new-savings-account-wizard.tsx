            ...
            <div className="savings-wizard-body">
              {charges.length === 0 ? (
                <div className="empty-state compact-empty">
                  <Coins size={24} />
                  <strong>No savings charges configured yet</strong>
                  <p>Optional opening and servicing charges will appear here once the backoffice templates are configured.</p>
                </div>
              ) : <div className="check-list">{charges.map((charge) => <label className="check-row" key={charge.id}><input checked={selectedCharges.has(charge.id)} onChange={() => toggleCharge(charge.id)} type="checkbox" />{charge.name}{charge.calculationType === "FLAT" ? ` \u00b7 ${formatMinor(BigInt(charge.amountMinor ?? "0"), charge.currencyCode)}` : ` \u00b7 ${((charge.percentageBps ?? 0) / 100).toFixed(2)}%`}</label>)}</div>}
            </div>
          ) : null}
