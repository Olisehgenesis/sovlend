...
export function EditClientForm({ client, accountNumber }: { client: EditableClient; accountNumber: string }) {
  ...
  return <form action={save} className="entity-form">
    ...
    <div className="form-actions"><BrandActionButton disabled={pending} icon={pending ? <LoaderCircle className="spin" size={18} /> : <Save size={18} />}>Save changes</BrandActionButton></div>
  </form>;
}
