...
| 9 | $500/day BTC cashier cap requiring manager approval above that | ❌ **MISSING (policy only)** | `src/modules/identity/domain/permissions.ts` — general `approvalLimitMinor` exists; nothing BTC-specific | Approval-limit infrastructure exists generically, but there is no BTC-specific daily cap or system-enforced manager gate. Today this is enforced by training and manual sign-off, not by the software. |
...
|---|---|---|---|---|
| 1 | System-enforced BTC daily cap | Add a BTC-specific daily transfer limit per cashier/role, enforced at the disbursement API layer (reject over-cap without manager approval, not just train around it) | **High — closes the biggest live risk** | Not started |
