#!/bin/sh
# Midnight PostgreSQL dump to Backblaze B2 as readable gzipped SQL.
#
#   postgres/daily/sovlend-daily-YYYY-MM-DD.sql.gz
#
# Keeps 21 days, then hard-deletes older files.
set -eu
...
echo "uploaded ${daily_name}"

rclone lsf "$remote_daily/" 2>/dev/null | grep '^sovlend-daily-[0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9]\.sql\.gz$' | sort -r | tail -n "+$((BACKUP_KEEP_DAILY + 1))" | while IFS= read -r name; do
  [ -n "$name" ] || continue
  echo "deleting ${remote_daily}/${name}"
  rclone deletefile "${remote_daily}/${name}" || true
  rclone deletefile "${remote_daily}/${name}.sha256" || true
done

# Drop leftover monthly copies from the previous retention scheme.
rclone delete "$remote_monthly" || true
